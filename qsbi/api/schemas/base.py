...
def check_one_non_null(values, *fields) -> Any:
    ...
    if not lst:
        lst = list(values.keys())
    if all(map(lambda x: is_null(mapget(values, x)), lst)):
        raise ValueError(f"at least one field from {lst} should not be null")
    return values
...
class BaseQsbi(BaseORM):  # basic class - change here to skip ORM
    pass
